...
export const accordionData = [
  ...
  const { dark , setIsDark, toggleDarkMode} = useContext(ModeContext)
    ...
    return (
      <div className={`accordion-item py-8 border-t ${dark ? "border-t-[#9B9B9B]" : "border-t-[#03045E]"}  cursor-pointer`}>
        ...
        {isActive && <div className={`font-secondary  text-left xl:text-base text-xs px-3 xl:mx-16 py-5  ${dark ? "text-gray-200 font-extralight" : "text-[#023E8A] font-medium"} `}>{content}</div>}
      </div>
    );
  };
...
export default function Faq(props) {
  ...
  return (
    <div className=' p-5 xl:px-52 flex flex-col justify-center mb-10 py-6 xl:py-20 mt-44 xl:mt-0' id='faq'>
        <h1 className={`font-primary text-center xl:text-left text-4xl xl:text-7xl  mb-5 ${dark ? "text-white" : "text-[#03045E]"}`}>FAQs</h1>
